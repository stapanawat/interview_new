...
export default function PositionManagement({ positions, onRefresh, currentUser }) {
  ...
  const remove = async (item) => {
    ...
    if (!response.ok) { const data = await response.json(); return alert(data.error || 'ไม่สามารถลบตำแหน่งงานได้'); }
    onRefresh();
  };
  ...
  return <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, .8fr) minmax(400px, 1.2fr)', gap: 20, alignItems: 'start' }}>
    <form className="pastel-card" onSubmit={submit}>
      ...
      <div className="form-group"><label>ชื่อตำแหน่ง *</label><input className="input-pastel" value={form.name} onChange={(e) => update('name', e.target.value)} required /></div>
      <div className="form-group"><label>แผนก</label><input className="input-pastel" value={form.department} onChange={(e) => update('department', e.target.value)} placeholder="เช่น Technology" /></div>
      <div className="form-group"><label>สถานะรับสมัคร</label><select className="input-pastel" value={form.status} onChange={(e) => update('status', e.target.value)}><option value="Open">เปิดรับสมัคร</option><option value="Closed">ปิดรับสมัคร</option></select></div>
      <div style={{ display: 'flex', gap: 8 }}><button className="btn-pastel btn-pastel-primary" disabled={saving} type="submit"><Plus size={16} />{saving ? 'กำลังบันทึก...' : editingId ? 'บันทึก' : 'เพิ่มตำแหน่ง'}</button>{editingId && <button className="btn-pastel btn-pastel-secondary" type="button" onClick={reset}>ยกเลิก</button>}</div>
    </form>
    <div className="pastel-card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      {positions.map((item) => <div key={item.id} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '16px 22px', borderBottom: '1px solid #F0F4F8' }}>
        ...
        <button className="btn-pastel btn-pastel-secondary" style={{ padding: '6px 9px' }} onClick={() => { setEditingId(item.id); setForm({ name: item.name, department: item.department || '', status: item.status }); }}><Edit size={15} /></button>
        <button className="btn-pastel btn-pastel-danger" style={{ padding: '6px 9px' }} onClick={() => remove(item)}><Trash2 size={15} /></button>
      </div>)}
